...
function wpm_6310_search_template($ids, $allSlider, $col){
	...
	function wpm_6310_social_icon($iconId, $url, $borderWidth, $memberId, $templateId, $contactInfo = null, $styleId = null, 	$totalIcon = 4){
		global $wpdb;
			...
			if($iconIds && $iconUrl){
				for($i = 0; $i < count($iconUrl); $i++){
					if($iconIds[$i] != "" && $iconUrl[$i] != ""){
						...
						if($selIcon){
							echo "<li><a " . wpm_6310_external_link($iconUrl[$i]) . "  title='{$selIcon['name']}'  id='wpm-social-link-{$memberId}-{$selIcon['id']}'><i class='" . $selIcon['class_name'] . "'></i></a></li>";
	...
	if ($data) {
		...
		if($contacts){
			foreach ($contacts as $contact) {
				...
				$str .= "<div class='wpm-custom-fields-list-{$ids}{$dataClass}' {$dataAttr}><div class='wpm-custom-fields-list-label-{$ids}'>" . str_replace("\\", "", $contact[0]) . "</div> <div class='wpm-custom-fields-list-content-{$ids}'>{$contact[1]}</div></div>";
			}    
		}       
	}
	if($str){
...
function wpm_6310_extract_member_description($file, $count, $id)
{
...
function wpm_team_showcase_supreme_install()
{
	...
	if (!($memberData)) {
		...
		$sql = "INSERT INTO {$member_table} (name, designation, profile_details_type, profile_url, open_new_tab, profile_details, effect, image, hover_image, iconids, iconurl, category, contact_info) VALUES
		('Adam Smith', 'CEO', '2', '', '0', '{$details}', 'top', '{$arr[0]}', '{$arr_hover[0]}', '1,37,24', 'https://www.linkedin.com||||admin@gmail.com||||+123456789', '{$category1}', '{$contact}'),
		('George Michel', 'Sales Agent', '2', '', '0', '{$details}', 'top', '{$arr[1]}', '{$arr_hover[1]}', '2,37,38', 'https://www.facebook.com||||admin@gmail.com||||+123456789', '{$category2}', '{$contact}'),
		('Margaret Thatcher', 'Web Developer', '2', '', '0', '{$details}', 'top', '{$arr[2]}', '{$arr_hover[2]}', '2,1,24', 'https://www.facebook.com||||https://www.linkedin.com||||+123456789', '{$category3}', '{$contact}'),
		('Aaron Hernandez', 'Web Developer', '2', '', '0', '{$details}', 'top', '{$arr[3]}', '{$arr_hover[3]}', '2,1,24', 'https://www.facebook.com||||https://www.linkedin.com||||+123456789', '{$category3}', '{$contact}'),
		('Lisa Haydon', 'Sales Agent', '2', '', '0', '{$details}', 'top', '{$arr[4]}', '{$arr_hover[4]}', '27,1,25', 'https://www.facebook.com||||admin@gmail.com||||+123456789', '{$category4}', '{$contact}')";
...
function wpm_6310_category_menu($categoryData, $styleId, $output = '')
{
